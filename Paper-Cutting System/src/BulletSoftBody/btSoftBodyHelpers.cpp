...
//Draw()這函式為主要畫物件外觀的地方，可控制格線顏色、格子顏色。
										...
										int drawflags)
{
	...
	const btVector3     lcolor = btVector3(0, 0, 0);  //線條顏色
...
#define USE_NEW_CONVEX_HULL_COMPUTER
	...
	else
	{
		...
		*/
		...
		{
			const btScalar  scl = (btScalar)1; // 0.8
			const btScalar  alp = (btScalar)1;
			const btVector3 col(1, (btScalar)1, 1); // 0, 0.7, 0
		...
		for (i = 0; i < psb->m_nodes.size(); ++i)
		{
			...
			if (n.route == 1)
			{
				//drawVertex(idraw, n.m_x, 0.25, btVector3(0, 0 , 0));
				GLUquadric *myQuad = gluNewQuadric();

				glPushMatrix();
				glColor4f( 0, 0, 0, 1 );
				glTranslatef( n.m_x.getX(), n.m_x.getY(), n.m_x.getZ() );
				gluSphere( myQuad, 0.05, 100, 100 );
				glPopMatrix();
			}
			/*
			if (n.tag == 1)  //face上面的點
