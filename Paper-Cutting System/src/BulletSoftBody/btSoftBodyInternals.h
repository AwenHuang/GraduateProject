...
struct btSymMatrix
{
	...
	int						index(int c,int r) const				{ if(c>r) btSwap(c,r);return((r*(r+1))/2+c); }
...
// 自建的Solve code
static btScalar				Solve(		  const btVector3& center,
								  const btVector3& a,
								  const btVector3& b,
								  ...
								  const int maxiterations=256)
{
	btScalar	span[2]={0,1};
	btScalar len = 0.15;
	//(x-impact).length2()-len*len
	btScalar	values[2]={(a-center).length2()-len*len,(b-center).length2()-len*len};
	...
	for(int i=0;i<maxiterations;++i)
	{
		...
		const btScalar	v=((Lerp(a,b,t)-center).length2()-len*len);
		if((t<=0)||(t>=1))		break;
										  ...
										  const int maxiterations=256)
{
	...
	for(int i=0;i<maxiterations;++i)
	{
		...
		//printf("v=%f\n",v);
		if((t<=0)||(t>=1))		break;
		if(btFabs(v)<accuracy)	return(t);  //取浮點數的絕對值
		if(v<0)
		{ span[0]=t;values[0]=v; }
		else
		{ span[1]=t;values[1]=v; }
