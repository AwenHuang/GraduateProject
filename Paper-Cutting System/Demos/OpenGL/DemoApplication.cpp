...
void DemoApplication::updateCamera()
{
	...
	else
	{
		...
		for ( int i = 0; i < 3; i++ )
		{
			cp[i]  = m_cameraPosition[i];
			ctp[i] = m_cameraTargetPosition[i];
			cu[i]  = m_cameraUp[i];
		}
	}
}
...
void DemoApplication::mouseFunc2(int button, int state, int x, int y)
{
	if (state == 0)
	{
